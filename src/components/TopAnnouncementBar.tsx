      ...
      >
        {fullLoopItems.map((item, idx) => (
          <div
            ...
            className="flex items-center gap-2 text-slate-200 hover:text-white transition-colors cursor-pointer whitespace-nowrap group/item shrink-0"
          >
            <span className="font-normal text-slate-200 group-hover/item:text-white transition-colors">
              {item.text}
            </span>
            {item.highlight && (
              <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-blue-500/20 border border-blue-400/40 text-blue-300 group-hover/item:text-blue-200 text-[10px] font-bold font-mono tracking-wider ml-1">
                {item.highlight} →
              </span>
            )}
          </div>
